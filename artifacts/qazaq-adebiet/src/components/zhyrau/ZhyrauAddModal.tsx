...
import { saveCustomZhyrau } from '@/hooks/useZhyrau';
...
const CENTURIES = ['XV', 'XVI', 'XVII', 'XVIII', 'XIX'];
const PERIODS = [
  'Ноғайлы дәуірі',
  'Қазақ хандығы',
  'Жоңғар шапқыншылығы',
...
const PROFESSIONS = ['Жырау', 'Ақын', 'Би', 'Батыр', 'Ел ақылшысы', 'Философ', 'Композитор'];
...
const EMPTY: FormData = {
  ...
  century: 'XVIII',
  era: 'XVIII ғасыр',
  historicalPeriod: 'Абылай хан дәуірі',
  khanPeriod: '',
  profession: ['Жырау'],
  description: '',
  biography: '',
...
export default function ZhyrauAddModal({ open, onClose, onSuccess }: Props) {
  ...
  const validateStep1 = () => {
    ...
    if (!form.fullName.trim()) e.fullName = 'Жырау атын енгізіңіз';
  ...
  const handleSave = () => {
    setSaving(true);
    const slug = slugify(form.fullName) || `zhyrau-${Date.now()}`;
    const century = form.century;
    ...
    const zhyrau: Zhyrau = {
      ...
      century,
      era: form.era || `${century} ғасыр`,
      historicalPeriod: form.historicalPeriod,
      khanPeriod: form.khanPeriod.trim() || 'Белгісіз',
      khanConnection: form.khanConnection.trim(),
      profession: form.profession.length > 0 ? form.profession : ['Жырау'],
      photo: '',
      coverImage: '',
      description: form.description.trim(),
      biography: form.biography.trim(),
      tolgauCount: parseInt(form.tolgauCount) || 0,
      ...
      pdf: [],
      mapLocation: {
        birthLabel: form.birthPlace.trim(),
        birthLat: 48.0,
        birthLng: 66.9,
        deathLabel: form.deathPlace.trim() || 'Белгісіз',
        deathLat: 48.0,
        deathLng: 66.9,
        livedRegions: [],
      },
            ...
            <div className="w-full max-w-2xl max-h-[90vh] flex flex-col bg-[#0f0a22] border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
                    ...
                    >
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <Field label="Толық аты-жөні" required>
                          <input
                            className={inputCls}
                            placeholder="Бұқар Қалқаманұлы"
                            value={form.fullName}
                            onChange={(e) => {
                              set('fullName', e.target.value);
                              if (!form.era)
                                set('era', `${form.century} ғасыр`);
                            }}
                          />
                          {errors.fullName && (
                        ...
                        <Field label="Туылған жылы" required>
                          ...
                          />
                          {errors.birthDate && (
                            <p className="text-red-400 text-xs">{errors.birthDate}</p>
                          )}
                        </Field>
                        ...
                        <Field label="Қайтыс болған жылы">
                          <input
                            className={inputCls}
                            placeholder="1781 (белгісіз болса бос қалдырыңыз)"
                        ...
                        <Field label="Туылған жері" required>
                          <input
                            className={inputCls}
                            placeholder="Баянауыл өңірі"
                            value={form.birthPlace}
                            onChange={(e) => set('birthPlace', e.target.value)}
                          />
                          {errors.birthPlace && (
                            <p className="text-red-400 text-xs">{errors.birthPlace}</p>
                          )}
                        </Field>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <Field label="Ғасыры">
                          <select
                            ...
                            value={form.century}
                            onChange={(e) => {
                              set('century', e.target.value);
                              set('era', `${e.target.value} ғасыр`);
                            }}
                          >
                            {CENTURIES.map((c) => (
                        ...
                        <Field label="Тарихи дәуірі">
                          ...
                          >
                            {PERIODS.map((p) => (
                              <option key={p} value={p} className="bg-[#0f0a22]">
                                {p}
                              </option>
                            ))}
                          </select>
                        ...
                        <Field label="Хан дәуірі">
                          <input
                            className={inputCls}
                            placeholder="Абылай хан дәуірі"
                            value={form.khanPeriod}
                            onChange={(e) => set('khanPeriod', e.target.value)}
                          />
                        </Field>
                      </div>
                    ...
                    >
                      <Field label="Қысқа сипаттама" required>
                        <textarea
                          ...
                          rows={3}
                          placeholder="Жырау туралы қысқаша (2-3 сөйлем)..."
                      ...
                      <Field label="Толық өмірбаяны" required>
                        <textarea
                          className={`${inputCls} resize-none`}
                          rows={10}
                          placeholder="Жырауды өмірбаянын толық жазыңыз. Жаңа абзацты Enter арқылы бөліңіз..."
                          value={form.biography}
                          onChange={(e) => set('biography', e.target.value)}
                        />
                        {errors.biography && (
                          <p className="text-red-400 text-xs">{errors.biography}</p>
                        )}
                      </Field>

                      <Field label="Хандықпен байланысы">
                        <textarea
                          className={`${inputCls} resize-none`}
                          rows={3}
                          placeholder="Хан немесе билеушімен қарым-қатынасы туралы..."
                          value={form.khanConnection}
                          onChange={(e) => set('khanConnection', e.target.value)}
                        />
                      </Field>
                    </motion.div>
                  )}
                  ...
                  {step === 3 && (
                    ...
                    >
                      <div className="grid grid-cols-2 gap-4">
                        <Field label="Толғаулар саны">
                          <input
                            type="number"
                            min={0}
                            className={inputCls}
                            placeholder="0"
                            value={form.tolgauCount}
                            onChange={(e) => set('tolgauCount', e.target.value)}
                          />
                        </Field>
                      ...
                      <Field label="Тегтер (үтірмен бөліп жазыңыз)">
                        <input
                          className={inputCls}
                          placeholder="XVIII ғасыр, Мектеп бағдарламасы, Жырау"
                          value={form.tags}
                      ...
                      <div className="glass-panel rounded-xl p-4 border border-primary/20">
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          <span className="px-2.5 py-0.5 rounded-full text-xs bg-accent/20 text-accent border border-accent/30">
                            {form.era || `${form.century} ғасыр`}
                          </span>
                          <span className="px-2.5 py-0.5 rounded-full text-xs bg-primary/20 text-primary border border-primary/30">
                            {form.historicalPeriod}
                          </span>
                          {form.profession.map((p) => (
                            <span
                              key={p}
              ...
              <div className="flex items-center justify-between px-6 py-4 border-t border-white/10 shrink-0">
                ...
                {step < 3 ? (
                  <button
                    onClick={next}
                    className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-medium bg-primary hover:bg-primary/90 text-white transition-colors shadow-lg shadow-primary/20"
                  >
                    Келесі
                  ...
                  >
                    ...
                    {saving ? 'Сақталуда...' : 'Жырауды сақтау'}
                  </button>
                )}
              </div>
            </div>
