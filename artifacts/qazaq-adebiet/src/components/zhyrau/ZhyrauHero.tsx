...
import type { ZhyrauFilter } from '@/types/zhyrau';

interface ZhyrauHeroProps {
  filter: ZhyrauFilter;
...
export default function ZhyrauHero({ filter, suggestions, setFilter, total }: ZhyrauHeroProps) {
  ...
  return (
    <section className="relative w-full min-h-[420px] flex items-center justify-center overflow-hidden bg-gradient-to-br from-[#0f0520] via-[#1a0540] to-[#0d1840] pt-20">
      <motion.div
        ...
        className="absolute top-10 left-1/4 w-96 h-96 bg-amber-600/20 rounded-full blur-[100px] pointer-events-none"
      />
      <motion.div
        animate={{ x: [0, -40, 0], y: [0, 50, 0] }}
        transition={{ duration: 12, repeat: Infinity, ease: 'easeInOut', delay: 2 }}
        className="absolute bottom-10 right-1/4 w-[400px] h-[400px] bg-violet-500/20 rounded-full blur-[120px] pointer-events-none"
      />
      ...
      <div className="container mx-auto px-4 relative z-10 flex flex-col items-center text-center">
        ...
        >
          ...
          <span className="text-sm font-medium text-white/90">Жыраулар энциклопедиясы</span>
        </motion.div>
        ...
        >
          Қазақ халқының жырауларының өмірі, тарихи дәуірі, толғаулары және әдеби мұрасы.
        </motion.p>
        ...
        <motion.div
          ...
          ref={containerRef}
        >
          <div className="relative glass-panel rounded-full overflow-hidden flex items-center px-4 py-2 ring-1 ring-white/20 focus-within:ring-primary/50 transition-all">
            <Search className="w-6 h-6 text-white/50 flex-shrink-0" />
            <input
              type="text"
              placeholder="Жыраудың атын немесе толғауын іздеңіз..."
              className="w-full bg-transparent border-none text-white px-4 py-3 focus:outline-none placeholder:text-white/40"
              value={filter.search}
              onChange={(e) => { setFilter({ search: e.target.value }); setShowSuggestions(true); }}
              ...
              >
                {suggestions.map((s) => (
                  <button
                    ...
                    onClick={() => { setFilter({ search: s.label }); setShowSuggestions(false); }}
                  >
                    {s.label}
                  </button>
                ))}
