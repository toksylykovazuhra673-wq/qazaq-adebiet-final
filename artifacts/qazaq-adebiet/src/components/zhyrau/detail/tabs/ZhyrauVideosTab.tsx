...
const YOUTUBE_SEARCH = (name: string) =>
  `https://www.youtube.com/results?search_query=${encodeURIComponent(name + ' жырау')}`;
      ...
      <div className="space-y-6">
        <div className="glass-panel rounded-2xl p-8 border border-white/5">
          <div className="flex flex-col items-center text-center py-8">
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-red-500/15 to-rose-600/10 border border-red-500/20 flex items-center justify-center mb-5">
              <Video className="w-9 h-9 text-red-400/60" />
            </div>
            <h3 className="text-xl font-serif text-white mb-2">Бейнематериалдар жоқ</h3>
            <p className="text-white/50 text-sm max-w-sm mb-6">
              {zhyrau.fullName} жырауға арналған дәрістер мен деректі фильмдер жақын арада қосылады.
            </p>
            <a
              href={YOUTUBE_SEARCH(zhyrau.fullName)}
              target="_blank"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            `${zhyrau.fullName} туралы деректі фильм`,
            `${zhyrau.fullName} толғаулары — дәріс`,
            `${zhyrau.era} жырлары мен толғаулары`,
          ].map((title, i) => (
            <div
              key={i}
              className="glass-card rounded-xl overflow-hidden opacity-40"
            >
              ...
              <div className="p-4">
                <p className="text-white/50 text-sm font-medium line-clamp-2">{title}</p>
                <p className="text-white/25 text-xs mt-1">Жақын арада...</p>
              </div>
            </div>
          ))}
        </div>
          ...
          <a href={v.url} target="_blank" rel="noopener noreferrer">
            <div className="relative aspect-video bg-white/5 flex items-center justify-center cursor-pointer">
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-black/30 group-hover:bg-black/10 transition-colors">
                <div className="w-14 h-14 rounded-full bg-red-600/80 flex items-center justify-center shadow-lg">
