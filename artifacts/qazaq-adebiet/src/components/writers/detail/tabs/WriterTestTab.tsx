...
import { ALL_WRITERS } from '@/hooks/useWriters';
...
function generateQuestions(writer: Writer): Question[] {
  const questions: Question[] = [];
  
  // Q1: Name
  const otherWriters = ALL_WRITERS.filter(p => p.id !== writer.id).map(p => p.fullName);
  const nameOptions = [writer.fullName];
  while (nameOptions.length < 4 && otherWriters.length > 0) {
    const randomIdx = Math.floor(Math.random() * otherWriters.length);
    nameOptions.push(otherWriters.splice(randomIdx, 1)[0]);
  }
  // Shuffle
  ...
  questions.push({
    id: 1,
    question: "Жазушының толық аты кім?",
    options: nameOptions,
    correctAnswer: writer.fullName
  });

  // Q2: Birth year
  const birthYearMatch = writer.birthDate.match(/\d{4}/);
  ...
  if (birthYear) {
    ...
    questions.push({
      id: 2,
      question: "Жазушы қай жылы туылды?",
      options: yearOptions,
  ...
  const movements = ["Алаш", "Классикалық әдебиет", "Кеңес дәуірі", "Тәуелсіздік кезеңі"];
  const movOptions = [writer.literaryMovement];
  movements.filter(m => m !== writer.literaryMovement).slice(0, 3).forEach(m => movOptions.push(m));
  movOptions.sort(() => Math.random() - 0.5);
  
  questions.push({
    id: 3,
    question: "Жазушының негізгі әдеби бағыты қандай?",
    options: movOptions,
    correctAnswer: writer.literaryMovement
  });

  // Q4: Works/Novels
  if (writer.works && writer.works.length > 0) {
    const mainWork = writer.works[0].title;
    ...
    questions.push({
      id: 4,
      question: "Төмендегілердің қайсысы жазушының атақты шығармасы?",
      options: workOptions,
      correctAnswer: mainWork
    });
  } else if (writer.novels && writer.novels.length > 0) {
    const mainWork = writer.novels[0].title;
    const fakeWorks = ["Қан мен тер", "Көшпелілер", "Абай жолы", "Ұлпан", "Ақиқат пен аңыз"];
    const workOptions = [mainWork, ...fakeWorks.slice(0, 3)].sort(() => Math.random() - 0.5);
    ...
    const profOptions = [writer.profession[0] || "Жазушы", "Мүсінші", "Архитектор", "Дәрігер"].sort(() => Math.random() - 0.5);
    questions.push({
      id: 4,
      question: "Жазушы қандай кәсіп иесі болды?",
      options: profOptions,
      correctAnswer: writer.profession[0] || "Жазушы"
    });
  }

  // Q5: Fact
  const trueFact = writer.interestingFacts?.[0] || `${writer.shortName} өте дарынды тұлға болған.`;
  const falseFacts = [
    `${writer.shortName} өмірінің соңына дейін шет елде өмір сүрді.`,
  ...
  questions.push({
    id: 5,
    question: "Жазушы туралы дұрыс тұжырымды таңдаңыз:",
    options: factOptions,
...
export default function WriterTestTab({ writer }: { writer: Writer }) {
  ...
  const questions = useMemo(() => generateQuestions(writer), [writer]);
    ...
    return (
      <div className="glass-panel rounded-3xl p-12 flex flex-col items-center text-center max-w-lg mx-auto">
        ...
        <p className="text-white/70 mb-8 text-lg">
          {writer.fullName} туралы біліміңізді 5 сұрақтан тұратын интерактивті тест арқылы тексеріп көріңіз.
        </p>
        <button
          onClick={handleStart}
