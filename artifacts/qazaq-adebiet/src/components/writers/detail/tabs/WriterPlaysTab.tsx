  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {writer.plays.map((play) => (
        <div key={play.id} className="glass-card p-5 rounded-xl border border-white/10 relative overflow-hidden">
          ...
          <p className="text-white/70 text-sm line-clamp-3">{play.description}</p>
        </div>
      ))}
    </div>
  );
