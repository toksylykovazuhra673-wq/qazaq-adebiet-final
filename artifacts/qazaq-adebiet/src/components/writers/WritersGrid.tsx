...
export default function WritersGrid({ writers, resetFilter, isLoading = false }: WritersGridProps) {
  if (isLoading) {
    return (
  ...
  if (writers.length === 0) {
    return (
      ...
      >
        ...
        <p className="text-white/60 mb-6">Сүзгі параметрлерін өзгертіп көріңіз немесе іздеуді тазалаңыз.</p>
        <button
          onClick={resetFilter}
