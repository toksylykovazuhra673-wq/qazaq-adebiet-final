...
import { saveCustomPoet } from '@/hooks/usePoets';
...
const CENTURIES = ['XV', 'XVI', 'XVII', 'XVIII', 'XIX', 'XX', 'XXI'];
const MOVEMENTS = [
  'Халық ақындары',
  'Ағартушылық',
  'Алаш',
...
const PROFESSIONS = ['Ақын', 'Жазушы', 'Жырау', 'Аудармашы', 'Драматург', 'Ғалым'];
...
const EMPTY: FormData = {
  ...
  nationality: 'Қазақ',
  century: 'XX',
  literaryMovement: 'Халық ақындары',
  profession: ['Ақын'],
  description: '',
  biography: '',
  quotesCount: '0',
...
export default function PoetAddModal({ open, onClose, onSuccess }: Props) {
  ...
  const validateStep1 = () => {
    ...
    if (!form.fullName.trim()) e.fullName = 'Ақынның толық атын енгізіңіз';
  ...
  const handleSave = () => {
    setSaving(true);
    const slug = slugify(form.fullName) || `poet-${Date.now()}`;
    ...
    const poet: Poet = {
      ...
      profession: form.profession.length > 0 ? form.profession : ['Ақын'],
      photo: '',
      coverImage: '',
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          >
            <div className="w-full max-w-2xl max-h-[90vh] flex flex-col bg-[#0f0520] border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
              {/* Header */}
              <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 shrink-0">
                <div>
                  <h2 className="text-lg font-serif text-white font-semibold">Жаңа ақын қосу</h2>
              ...
              <div className="flex gap-0 shrink-0">
                {[1, 2, 3].map((s) => (
                  <div
                    key={s}
                    className={`h-1 flex-1 transition-colors ${s <= step ? 'bg-primary' : 'bg-white/10'}`}
                  />
                ))}
              </div>
                    ...
                    >
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <Field label="Қайтыс болған жылы">
                          <input
                            className={inputCls}
                            placeholder="1904 (тірі болса бос қалдырыңыз)"
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <Field label="Ғасыры">
                          ...
                          >
                            {CENTURIES.map((c) => (
                              <option key={c} value={c} className="bg-[#0f0520]">
                                {c} ғасыр
                              </option>
                            ))}
                          </select>
                        ...
                        <Field label="Әдеби ағым">
                          ...
                          >
                            {MOVEMENTS.map((m) => (
                              <option key={m} value={m} className="bg-[#0f0520]">
                                {m}
                              </option>
                            ))}
                          </select>
                    ...
                    >
                      <Field label="Қысқа сипаттама" required>
                        <textarea
                          ...
                          rows={3}
                          placeholder="Ақын туралы қысқаша (2-3 сөйлем)..."
                      ...
                      <Field label="Толық өмірбаяны" required>
                        <textarea
                          className={`${inputCls} resize-none`}
                          rows={10}
                          placeholder="Ақынның өмірбаянын толық жазыңыз. Жаңа абзацты Enter арқылы бөліңіз..."
                  ...
                  {step === 3 && (
                    ...
                    >
                      ...
                      <Field label="Тегтер (үтірмен бөліп жазыңыз)">
                        <input
                          className={inputCls}
                          placeholder="XIX ғасыр, Мектеп бағдарламасы, Ақын"
                          value={form.tags}
                      ...
                      <div className="glass-panel rounded-xl p-4 border border-primary/20">
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                          ...
                          {form.profession.map((p) => (
                            <span key={p} className="px-2.5 py-0.5 rounded-full text-xs bg-white/5 text-white/60 border border-white/10">
                              {p}
                            </span>
                          ))}
                        </div>
              ...
              <div className="flex items-center justify-between px-6 py-4 border-t border-white/10 shrink-0">
                ...
                {step < 3 ? (
                  ...
                  >
                    ...
                    {saving ? 'Сақталуда...' : 'Ақынды сақтау'}
                  </button>
                )}
              </div>
            </div>
