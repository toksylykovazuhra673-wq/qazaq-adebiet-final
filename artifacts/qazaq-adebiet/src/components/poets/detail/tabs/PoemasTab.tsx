  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {poet.poemas.map((poema) => (
        <div key={poema.id} className="glass-card rounded-2xl p-6 relative group overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
            <BookMarked className="w-24 h-24 text-primary" />
          </div>
          
          <div className="relative z-10">
            <div className="inline-block px-3 py-1 bg-white/5 border border-white/10 rounded-full text-xs text-accent mb-4">
              {poema.year}
            </div>
