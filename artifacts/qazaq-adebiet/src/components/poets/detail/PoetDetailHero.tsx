...
import { ArrowLeft, Calendar, BookOpen, Eye, MapPin } from 'lucide-react';
import { Link } from 'wouter';
import type { Poet } from '@/types/poet';

export default function PoetDetailHero({ poet }: { poet: Poet }) {
  return (
    <div className="relative w-full overflow-hidden border-b border-white/10 bg-black/20">
      ...
      <div className="container mx-auto px-4 lg:px-8 py-10 lg:py-16 relative z-10">
        ...
        >
          ...
          <span className="text-sm font-medium">Барлық ақындар</span>
        </Link>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              <div className="flex items-start gap-3 glass-panel p-4 rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-sm text-white font-medium">{poet.birthDate}</p>
                  <p className="text-xs text-white/60 mt-0.5 flex items-center gap-1">
                    <MapPin className="w-3 h-3" /> {poet.birthPlace}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 glass-panel p-4 rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-sm text-white font-medium">{poet.deathDate ?? 'Тіршілікте'}</p>
                  {poet.deathPlace && (
                    <p className="text-xs text-white/60 mt-0.5 flex items-center gap-1">
                      <MapPin className="w-3 h-3" /> {poet.deathPlace}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 glass-panel p-4 rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-sm text-white font-medium">{poet.worksCount} шығарма, {poet.quotesCount} нақыл</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 glass-panel p-4 rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-sm text-white font-medium">{poet.viewCount.toLocaleString('kk-KZ')} рет қаралды</p>
                </div>
              </div>
            </div>
