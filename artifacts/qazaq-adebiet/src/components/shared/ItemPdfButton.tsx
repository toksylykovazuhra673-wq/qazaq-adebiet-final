 ...
 * Uses useWorkPdf hook (localStorage) to store one PDF per item.
 */
        ...
        {tab === 'url' ? (
          <>
            <input
              ...
              onKeyDown={e => { if (e.key === 'Enter' && validateUrl(url)) onSave(url.trim(), url.trim().split('/').pop() ?? 'PDF'); if (e.key === 'Escape') onCancel(); }}
            ...
            <div className="flex gap-1.5">
              ...
              <button onClick={() => { if (validateUrl(url)) onSave(url.trim(), url.trim().split('/').pop() ?? 'PDF'); }}
  ...
  if (variant === 'icon') {
    return (
      <div className="relative inline-flex items-center gap-1">
        <button
          title={hasPdf ? 'PDF оқу' : 'PDF қосу (сілтеме немесе файл)'}
        ...
        <AnimatePresence>
          {showViewer && entry && <PdfViewer url={entry.url} title={entry.title} onClose={() => setShowViewer(false)} />}
        </AnimatePresence>
      </div>
    );
  }
      ...
      {hasPdf ? (
        <>
          <button onClick={() => setShowViewer(true)}
            ...
            <FileText className="w-3.5 h-3.5" />
            {isFile ? '📂 PDF оқу' : 'PDF оқу'}
          </button>
          <button onClick={e => { e.stopPropagation(); setShowForm(s => !s); }}
            className="px-2.5 py-1.5 rounded-lg text-xs bg-white/5 hover:bg-white/10 text-white/35 border border-white/8 transition-colors" title="Өзгерту">
            ✎
          </button>
