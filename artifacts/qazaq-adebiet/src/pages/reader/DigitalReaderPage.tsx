...
export default function DigitalReaderPage() {
  ...
  const {
    ...
    setTextSettings, toggleFavorite,
    addBookmark, removeBookmark, isBookmarked,
    addNote, updateNote, removeNote,
  } = useDigitalReader(slug);

  // Navigate to correct tab when clicking a bookmark
  const handleNavigateBookmark = useCallback((bm: DrBookmark) => {
    if (bm.type === 'text')  { setActiveTab('text');  /* scroll handled by TextReaderTab restore */ }
  ...
  // Determine current source for Notes tab
  const noteSource = (['text', 'pdf', 'audio'] as const).includes(persist.activeTab as any)
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {persist.activeTab === 'characters' && (
            <CharactersTab book={book} />
          )}

          {persist.activeTab === 'summary' && (
            <SummaryTab book={book} />
          )}

          {persist.activeTab === 'facts' && (
            <FactsTab book={book} />
          )}
